...
nulluser()				/* babysit CPU when no one is home */
{
	...
	sysinit();
	// kprintf("sysinit successful hua bhaijaannnnnnn!!!");
...
sysinit()
{
	...
	struct	mblock	*mptr;
	SYSCALL pfintr();
	// kprintf("Page fault handler se toh bahar aa gaye! Shukar hai!");
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	{
		get_frm(&new_frame);
		// kprintf("Oye thakur! Kitne get frames hue!??");
		frm_tab[new_frame].fr_status = FRM_MAPPED;
		frm_tab[new_frame].fr_type = FR_TBL;
		frm_tab[new_frame].fr_pid = NULLPROC;

		pte = (FRAME0 + new_frame) * NBPG;
		...
		for (j = 0; j < 1024; j++)
		{
			...
			(pte + j)->pt_global = 1;
			(pte + j)->pt_avail = 0;
			(pte + j)->pt_base = i * FRAME0 + j;
		}
	}

	/* global page tablesy*/
	get_frm(&new_frame);
	proctab[NULLPROC].pdbr = (FRAME0 + new_frame) * NBPG;
	frm_tab[new_frame].fr_status = FRM_MAPPED;
	frm_tab[new_frame].fr_type = FR_DIR;
	frm_tab[new_frame].fr_pid = NULLPROC;

	pd_t* pde = proctab[NULLPROC].pdbr;
	...
	set_evec(14,(u_long)pfintr); /* ISR for page fault at number 14  */
